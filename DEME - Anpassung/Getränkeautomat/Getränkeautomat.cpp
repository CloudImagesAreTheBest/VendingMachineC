...
double dGeldEinwurf(double Geldforderung)
{
	...
	cout << endl << "Sie haben " << Muenzwert << "Euro eingeworfen." << endl;
...
// Calculates the check number and checks it with the user input
int Pruefziffer()
{
...
// Displays the Vending Machine Selection Menu
double Auswahl()
{
	...
	int iInnerLoopFailure = 0, iOuterLoopFailure = 0;
	do {
		Geldforderung = 0.0;
		fflush(stdin);
		cin >> Kategorie1;
	...
	} while (iOuterLoopFailure == 1);
	return Geldforderung;
}


// Converting Char´s to Int
